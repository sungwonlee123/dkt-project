...
def visualize_skillbuilder_attention_patterns_enhanced(model, dataset, sequence_length=None, basic_matrix=None, original_matrix=None, problem_names=None):
    """
    Skill Builder 데이터셋 전용 개선된 Attention 패턴 시각화 함수 (DKT Old, DKT2)
    - Basic 결과를 가져와서 정규화 전/후 비교 표시
    """
    print("\nAnalyzing enhanced Skill Builder attention patterns...")
    ...
    original_matrix_real = original_matrix.copy()  # 실제 원본 매트릭스 (정규화 전)
    
    
    print(f"\nSkill Builder 개선된 어텐션 값 분석:")
    ...
    # 원본 값 히트맵 (왼쪽) - Skill Builder 전용 범위
    ...
    ax1.set_title('어텐션 영향력 (원본 값) - Skill Builder', pad=20, fontsize=14)
    ...
    ax2.set_title('어텐션 영향력 (정규화) - Skill Builder', pad=20, fontsize=14)
    ...
    # 전체 제목
    fig.suptitle('Skill Builder 데이터셋: 전체 학생 데이터 기반 문제 간 영향력 분석 (원본 vs 정규화)', fontsize=16, y=0.98)
