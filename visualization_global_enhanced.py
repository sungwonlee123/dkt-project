...
def visualize_global_attention_patterns_enhanced(model, dataset, sequence_length=None, basic_matrix=None, original_matrix=None, problem_names=None):
    """
    개선된 전체 데이터셋에 대한 Attention 패턴 시각화 함수
    - Basic 결과를 가져와서 정규화 전/후 비교 표시
    """
    print("\nAnalyzing enhanced global attention patterns...")
    ...
    ax1.set_title('어텐션 영향력 (원본 값)', pad=20, fontsize=14)
    ...
    ax2.set_title('어텐션 영향력 (정규화)', pad=20, fontsize=14)
    ...
    # 전체 제목
    fig.suptitle('전체 학생 데이터 기반 문제 간 영향력 분석 (원본 vs 정규화)', fontsize=16, y=0.98)
